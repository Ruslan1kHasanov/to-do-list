...
const Task = (props) => {
      ...
      return(
            ...
            <div className="task_wrapper">
                  <div className="task_container" onClick={() => {props.show_hide_view_task_menu(local_state.task.id_note)}}>
                        <h3 className='proj_name'>{local_state.task.sub_project_name}</h3>
                        <div className="date_of_deadline noselect">{local_state.task.date_of_deadline}</div>
                        <p className="task_short_text">
                              {local_state.task.short_text}
                        </p>
                        <div className="peoples_placeholder">
                              { developers_arr }
                        </div>
                        <div className={`task_priority_${local_state.task.priority}` +
                                          ' task_priority noselect'}>{local_state.task.priority}</div>
                  </div>
            </div>
            </>
      );
